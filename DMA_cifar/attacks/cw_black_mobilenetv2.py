...
def check_optimizer(indice,grad, model, input_var,target, modifier_var, up, down, lr, is_target,use_tanh):
    ...
    m_copy[indice] -= lr * torch.sign(grad)
    if use_tanh:
    ...
    #calculate v3 
    m_copy[indice] += 2 * lr * torch.sign(grad)
    if use_tanh:
        input_adv = (tanh_rescale(modifier_var_copy + input_var, 0,0) + 1)/ 2
    else:
        input_adv = modifier_var_copy + input_var
    ...
    if real_v2 <= default_real :
        old_val -= lr * torch.sign(grad)
    if real_v3 <= default_real :
        old_val += lr * torch.sign(grad)
...
class BlackBoxL2:
    ...
    def _optimize(self, model, meta_model, step, input_var, modifier_var, target_var, scale_const_var, target, batch_idx, indice,input_orig = None):
        ...
        if self.clamp_fn == 'tanh':
            input_adv = tanh_rescale(modifier_var + input_var, self.clip_min, self.clip_max) /_out
        else:
        ...
        output = F.softmax(model(input_adv), dim = 1)  # output
        ...
        meta_optimizer = optim.Adam(meta_model.parameters(), lr = 0.01) # 选择优化器
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        input_adv_copy = copy.deepcopy(input_adv.detach()) # deepcopy:copy后的input_adv_copy与input_adv是互相独立的


        if guided and step==0:
            meta_output = meta_model(input_adv.detach())
            indice = torch.abs(meta_output.data).cpu().numpy().reshape(-1).argsort()[-500:] # 这里就是top-q cifar的top-q=500



        if (step + 1) % every_iter  == 0 and step < max_iter:
            # finetuning代码在indice生成代码之前，
            # 这表示要是当 (step + 1) % every_iter  == 0 and step < max_iter成立时
            # 当前的indice还是上一次攻击过程的indice即上个迭代的top-q梯度对应的像素
            # 此时ZOO方法估计对应的indice(top-q)像素的梯度
            # 那么self._optimize()接受indice，返回indice就有意义了

        # if True:
            #target = model(input_adv).argmax(dim=1)
            # 这里就是fine-tuning
            zoo_gradients = []
            generate_grad = generate_gradient(device, targeted = self.targeted) # 定义函数
            
            zoo_grad,select_indice = generate_grad.run(model, input_adv_copy, target,indice) # select_indice就是indice
        ...
        # 每次循环(噪声优化)都有：元攻击者估计的top update pixels个 梯度
        # 每次优化都会更新indice，就是说每次优化都会攻击不同的像素
        # 每次优化的梯度都是估计出来(ZOO或meta)的top indice个，改变每次优化的梯度分布(数量（先多后少）、不是每次都是top-indice的而选择估计梯度较小的像素)

        indice2 = indice

        # 每次循环都要选择哪个梯度估计器的梯度
        # 每一个间隔的迭代就用耗费巨大的ZOO梯度
        # 其余迭代用元攻击者的估计梯度
        if (step + 1) % every_iter  == 0 and step < max_iter:
        ...
        # 优化对抗样本
        # 这里不需要返回值，因为是python的对象都是指针
        # 修改的对象是噪声(modifier_var)
        ...
        return loss_np, loss1, loss2, dist_np, output_np, input_adv_np,indice,step_query_bias
    ...
    def run(self, model, meta_model, input, target, batch_idx = 0):
        ...
        for search_step in range(self.binary_search_steps):
            ...
            for step in range(self.max_steps):
                ...
                loss, loss1, loss2, dist, output, adv_img,indice,step_query_bias = self._optimize(
                    model,
                    meta_model,
                    step,
                    input_var,
                ...
                if self.abort_early and step % (self.max_steps // 100) == 0:
                    # if loss > prev_loss * .9999 or loss ==0:
                    # if loss ==0:
                    if first_step !=0:
        ...
        if first_step>max_iter:
            first_step = max_iter
        return o_best_attack, scale_const, first_step,best_query_bias
